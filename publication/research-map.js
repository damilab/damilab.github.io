(() => {
  ...
  function mount(papers) {
    ...
    const arrangeButton = host.querySelector('[data-arrange]'), arrangeExtra = host.querySelector('[data-arrange-extra]');
    let shown = [];
    ...
    function render() {
      host.querySelector('.rm').classList.toggle('is-arranging', arranging); arrangeButton.textContent = arranging ? 'Done arranging' : 'Arrange nodes'; arrangeExtra.hidden = !arranging;
      ...
      shown.forEach(node => {
        ...
        const g = document.createElementNS('http://www.w3.org/2000/svg', 'g'); g.setAttribute('class', `rmnode${arranging ? ' is-arranging' : ''}`); g.dataset.id = node.id; g.setAttribute('role', 'button'); g.setAttribute('tabindex', '0');
        ...
        if (arranging) {
          g.onpointerdown = event => {
            event.preventDefault(); event.stopPropagation(); const start = svgPoint(event), origin = { x: node.x, y: node.y };
            g.setPointerCapture?.(event.pointerId);
            const move = moving => { const point = svgPoint(moving), dx = point.x - start.x, dy = point.y - start.y; g.setAttribute('transform', `translate(${dx} ${dy})`); };
            const finish = ending => { const point = svgPoint(ending); node.x = Math.max(92, Math.min(748, origin.x + point.x - start.x)); node.y = Math.max(55, Math.min(378, origin.y + point.y - start.y)); layout[node.id] = { x: +node.x.toFixed(1), y: +node.y.toFixed(1) }; saveLayout(); g.removeEventListener('pointermove', move); g.removeEventListener('pointerup', finish); g.removeEventListener('pointercancel', finish); render(); };
            g.addEventListener('pointermove', move); g.addEventListener('pointerup', finish); g.addEventListener('pointercancel', finish);
          };
        }
      });
      svg.insertAdjacentHTML('beforeend', `<text class="rm-legend" x="84" y="386">${arranging ? 'Drag nodes to arrange · saved on this device' : '1 node = 1 paper · lines = shared topic or nearby research thread'}</text>`);
      renderInspector();
    }
