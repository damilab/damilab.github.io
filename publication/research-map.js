(() => {
  ...
  function mount(papers) {
    ...
    const yearButtons = host.querySelector('.rmy'), left = host.querySelector('.rm-controls'), inspector = host.querySelector('.rm-inspector'), svg = host.querySelector('svg');
    let shown = [];
    ...
    function renderInspector() {
      ...
      if (selected && active.some(n => n.id === selected.id)) {
        const venue = venueFor(selected), links = linksFor(selected);
        inspector.innerHTML = `<div class="rm-detail"><button aria-label="Close">×</button><small>${escape(selected.map_label || selected.subfield)} · ${venue ? `${escape(venue)} · ` : ''}${selected.year}</small><strong>${escape(selected.title)}</strong><p>${escape(selected.abstract || '').slice(0, 170)}${(selected.abstract || '').length > 170 ? '…' : ''}</p><div class="rm-detail-actions"><button data-show-card>전체 항목 보기</button>${links.map(l => `<a href="${escape(l.href)}" target="_blank" rel="noopener">${escape(l.name)}</a>`).join('')}</div></div>`;
        inspector.querySelector('[aria-label="Close"]').onclick = () => { selected = null; renderInspector(); };
        inspector.querySelector('[data-show-card]').onclick = () => scrollToCard(selected);
        return;
      }
      selected = null;
      ...
      inspector.querySelectorAll('[data-paper-id]').forEach(button => button.onclick = () => { selected = nodes.find(n => n.id === button.dataset.paperId); renderInspector(); });
    }
    function drawLinks(active) {
      ...
      picked.forEach(({ node, other, shared, sameSubfield }) => {
        ...
        line.setAttribute('stroke-width', shared ? '2.1' : sameSubfield ? '1.8' : '1.45'); line.setAttribute('stroke-opacity', shared ? '.82' : sameSubfield ? '.68' : '.52'); layer.append(line);
      });
      svg.append(layer);
    }
    ...
    function showNode(node) { selected = node; renderInspector(); }
    function render() {
